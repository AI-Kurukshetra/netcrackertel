...
export async function GET(request: NextRequest) {
  ...
  const auth = authorize({ role: sessionUser.role, tenantId: sessionUser.tenantId }, "orders");
  if (!auth.ok) {
  ...
  if (!DEMO_MODE) {
    ...
    const data = await getOrdersSupabase(supabase, url.searchParams.get("search") ?? "");
  ...
  return apiHandler("orders")(request);
}
