...
import { getDocumentWorkspace } from "@telecosync/services";
import { getSessionUserFromRequest } from "@/lib/auth";
import { DEMO_MODE } from "@/lib/data-mode";
import { getSupabaseServerClient } from "@/lib/supabase-server";
import { getDocumentsWorkspaceSupabase } from "@/lib/supabase-data";
...
export async function GET(request: NextRequest) {
  ...
  if (!DEMO_MODE) {
    ...
    const data = await getDocumentsWorkspaceSupabase(supabase, url.searchParams.get("search") ?? "");
  ...
  return NextResponse.json({
    resource: "documents",
    timestamp: new Date().toISOString(),
    ...getDocumentWorkspace(url.searchParams.get("search") ?? "")
  });
}
